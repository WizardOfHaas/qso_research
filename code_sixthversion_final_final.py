...
#pp1 = PdfPages('Original_sixth_400-500.pdf')
#final_pp = PdfPages('all_normalized.pdf')
#original_hellos=0
#normalized_hellos=0
#'pp'= PdfPages('original_sixth_many_graphs.pdf')
#pp2 = PdfPages('normalized_sixth_many_graphs.pdf')

#pp2 = PdfPages('normalized_sixth_no_original_graph.pdf')
#pp3=PdfPages('smooth_normalized.pdf')
#pp2 = PdfPages('Normalized.pdf')

spectra = loadtxt('DR9Q_selection_specnames.lst',dtype='str') #Load a list of spectra names
spectra_action = spectra[:] #Make it into an array

redshifts = loadtxt('DR9Q_selection_zem.lst') #Load a list of spectra redshifts
redshifts_action = redshifts[:] #Make it into an array

good_categorized_spectra = loadtxt('good_categorized_spectra_through_code_subtracted_one.txt')
...
for qqqqq in good_categorized_spectra_action:
    ...
    try:
    	pars,covar = curve_fit(powerlaw,power_law_datax,power_law_datay,p0=init_pars)
    except RuntimeError:
    ...
    try:
	pars2,covar = curve_fit(powerlaw2,power_law_datax2,power_law_datay2,p0=init_pars2)
	...
	text(wavelength_observe1-50 ,np.max(flux)-5,"z = " +`j`)
    ...
    ylabel("Flux[10^[-17]]cgs")
    text(wavelength_observe1 -50,np.max(normalizing)-0.2,"z = " +`j`)
